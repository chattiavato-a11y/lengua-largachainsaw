...
const DEFAULT_STT_MODEL = "@cf/openai/whisper-large-v3";
...
function selectSttModel(env, prefer){
  const pref = typeof prefer === "string" ? prefer.toLowerCase() : "";
  if (pref && env[`AI_STT_${pref.toUpperCase()}`]) return env[`AI_STT_${pref.toUpperCase()}`];
  if (pref === "premium" && env.AI_STT_PREMIUM) return env.AI_STT_PREMIUM;
  if (pref === "tiny" && env.AI_STT_TINY) return env.AI_STT_TINY;
  if (pref === "lite" && env.AI_STT_LITE) return env.AI_STT_LITE;
  if (env.AI_STT_DEFAULT) return env.AI_STT_DEFAULT;
  if (env.AI_STT) return env.AI_STT;
